...
export function Experience(){const root=usePageMotion();
 return <div ref={root}><a className="skip-link" href="#manifesto">Pular para o conteúdo</a><Header/><main><CampaignHero/><Chapters/></main><footer className="footer"><div className="footer-main"><a href="#inicio" className="footer-name">{site.name.toUpperCase()}</a><div><a href={site.instagram} target="_blank" rel="noreferrer">Instagram <ArrowUpRight size={14}/></a>{(site.whatsapp||site.whatsappUrl)&&<a href={appointmentUrl} target="_blank" rel="noreferrer">WhatsApp <ArrowUpRight size={14}/></a>}{site.phone&&<a href={`tel:${site.phone.replace(/[^\d+]/g,'')}`}>{site.phone}</a>}</div><div>{site.address&&<p>{site.address}</p>}{site.cro&&<p>{site.cro}</p>}<a href="/pre-anamnese">Preencher pré-anamnese ↗</a><a href="#inicio">Voltar ao início ↑</a></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} {site.name}</span><span>HARMONIZAÇÃO FACIAL</span></div></footer></div>
}
