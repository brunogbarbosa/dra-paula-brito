...
export function PreAnamnese() {
  ...
  return <div className={styles.page}>
        ...
        <div className={styles.stage} key={step}>
          ...
          {step < 3 ? <form onSubmit={submit} className={styles.form} inert={!ready}>
            ...
            <div className={styles.handoff}><MessageCircle size={25} /><h3>Vamos continuar<br /><em>pelo WhatsApp?</em></h3><p>{whatsappUrl ? 'A conversa será aberta com o resumo preenchido. Toque em enviar no WhatsApp para compartilhar.' : 'Copie suas respostas, abra a conversa da clínica e cole a mensagem para enviar.'}</p>
              ...
              {consent ? <a className={styles.primary} href={whatsappUrl || appointmentUrl} target="_blank" rel="noreferrer" onClick={() => setStatus('A conversa foi solicitada. O envio só acontece quando você confirmar no WhatsApp.')}><MessageCircle size={18} /> Abrir WhatsApp <ArrowUpRight size={17} /></a> : <button type="button" className={styles.primary} disabled><MessageCircle size={18} /> Abrir WhatsApp</button>}
    ...
    <footer className={styles.footer}><span>DRA. PAULA BRITO · {site.cro}</span><a href={appointmentUrl} target="_blank" rel="noreferrer">Prefere conversar com a equipe? <ArrowUpRight size={15} /></a></footer>
  </div>;
}
