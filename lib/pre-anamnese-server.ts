import 'server-only';
import { healthQuestions, normalizeBrazilPhone, type Intake } from './pre-anamnese';
...
export function validateSubmission(value: unknown) {
  ...
  const whatsapp = normalizeBrazilPhone(text(input.intake.whatsapp, 30));
  const city = text(input.intake.city, 40);
  if (name.length < 2 || !whatsapp || !Number.isInteger(age) || age < 1 || age > 120 || !['Recife', 'Surubim', 'Ainda não decidi'].includes(city)) return null;
  ...
  return { submission_id: text(input.submissionId, 36), name, whatsapp, age, city, interests, objective: text(input.intake.objective, 500), health, notes: text(input.intake.notes, 350), consent_at: new Date().toISOString() };
}
